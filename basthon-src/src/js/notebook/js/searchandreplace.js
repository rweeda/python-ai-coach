define(["jquery", "base/js/dialog", "base/js/i18n"], function (
  $,
  dialog,
  i18n,
) {
   ...
   **/
  var compute_preview_model = function (
    sre,
    arr,
    isCaseSensitive,
    RegExpOrNot,
    ...
    for (var r = 0; r < arr.length; r++) {
      ...
      for (var mindex = 0; mindex < matches.length; mindex++) {
        ...
        html.push([
          cutBefore(current_line.slice(0, start)),
          initial,
          replaced,
  ...
  var build_preview = function (body, aborted, html, replace) {
    ...
    if (aborted) {
      ...
      var matchmsg = i18n.msg.sprintf(
        i18n.msg.ngettext("%d match", "%d matches", html.length),
        html.length,
      );
  ...
  var snr = function (env, event) {
    ...
    var onError = function (body) {
      ...
      body.append(
        $("<p/>").text(
          i18n.msg._("No matches, invalid or empty regular expression"),
        ),
      );
    };
  ...
  var load = function (keyboard_manager) {
    ...
    var act_all = keyboard_manager.actions.register(
      action_all,
      "find-and-replace",
      "jupyter-notebook",
    );
