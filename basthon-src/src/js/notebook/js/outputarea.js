...
define([
  ...
  /**
   * helper class to manage asynchronicity in append mechanism
   * (mainly to trigger latex typseting after insertion in markdown cells)
   */
  class PromiseBunch {
    constructor() {
      this._promises = [];
  ...
  var OutputArea = function (options) {
    ...
    this.notebook = this.keyboard_manager.notebook;
  ...
  var MIME_DYNAMIC_IFRAME = "text/dynamic-iframe";
  var MIME_KERNEL_IFRAME = "text/json-kernel-iframe";
  ...
  OutputArea.prototype._append_javascript_error = function (err, element) {
    ...
    element
      ...
      .append(
        $("<div/>")
          .text(
            i18n.msg._("See your browser Javascript console for more details."),
          )
          .addClass("js-error"),
      );
  };
  ...
  OutputArea.prototype.append_execute_result = function (json) {
    ...
    const bunch = new PromiseBunch();
    var inserted = this.append_mime_type(json, toinsert, bunch.add());
    if (inserted) {
    ...
    if (
      ...
      json.data[MIME_DYNAMIC_IFRAME] !== undefined ||
      json.data[MIME_KERNEL_IFRAME] !== undefined ||
      json.data[MIME_MARKDOWN] !== undefined
    ) {
      bunch.all().then(() => this.typeset());
    }
  };
  ...
  OutputArea.prototype.append_stream = function (json) {
    // [Basthon]
    if (Array.isArray(json.text)) json.text = json.text.join("");
    var text = json.text;
  ...
  OutputArea.prototype.update_display_data = function (json, handle_inserted) {
    ...
    targets.map(function (target) {
      ...
      if (oa.append_mime_type(json, toinsert, bunch.add(handle_inserted))) {
  ...
  OutputArea.prototype.append_display_data = function (json, handle_inserted) {
    ...
    this._record_display_id(json, toinsert);
    const bunch = new PromiseBunch();
    if (this.append_mime_type(json, toinsert, bunch.add(handle_inserted))) {
      this._safe_append(toinsert);
      // If we just output latex, typeset it.
      if (
        json.data[MIME_LATEX] !== undefined ||
      ...
      if (json.data[type] !== undefined && append) {
        ...
        if (!this.trusted && !OutputArea.safe_outputs[type]) {
          ...
          if (type === MIME_HTML || type === "text/svg") {
            var parsed = $(security.sanitize_html_and_parse(value, true));
            toinsert = append.apply(this, [
              parsed,
              md,
              element,
              handle_inserted,
        ...
        if (
          [MIME_PNG, MIME_JPEG, MIME_GIF, MIME_MARKDOWN].indexOf(type) < 0 &&
  ...
  var append_dynamic_iframe = function (html, md, element) {
    /* typically triggerred by tutor */
    // data url ensure security isolation
    const dataURL = `data:text/html;charset=utf-8,${encodeURIComponent(html)}`;
    html = `<iframe allowFullscreen class="basthon-dynamic-iframe" frameborder="0" src="${dataURL}"></iframe>`;
    const toinsert = append_html.call(this, html, md, element);
    const iframe = toinsert.find("iframe")[0];
    this.notebook?.basthonGUI?.handleDynamicIFrame?.(iframe);
    return toinsert;
  };

  var append_kernel_iframe = function (json, md, element) {
    /* typically triggerred by matplotlib */
    let { key, html } = JSON.parse(json);
    const ports = window._basthonBypassBus.pop(key);
    // data url ensure security isolation
    const dataURL = `data:text/html;charset=utf-8,${encodeURIComponent(html)}`;
    html = `<iframe allowFullscreen frameborder="0" src="${dataURL}"></iframe>`;
    const toinsert = append_html.call(this, html, md, element);
    const iframe = toinsert.find("iframe")[0];
    this.notebook?.basthonGUI?.handleKernelIFrame?.(iframe, ports);
    return toinsert;
  };

  var append_markdown = function (markdown, md, element, handle_inserted) {
  ...
  OutputArea.prototype.clear_output = function (wait, ignore_clear_queue) {
    if (Basthon.pendingInput() && !Basthon.safeKernel())
  ...
  OutputArea.append_map[MIME_DYNAMIC_IFRAME] = append_dynamic_iframe;
  OutputArea.append_map[MIME_KERNEL_IFRAME] = append_kernel_iframe;
