...
define([
  ...
  KernelSelector.prototype.request_kernelspecs = function () {
    ...
    utils
      .promising_ajax(url, settings)
      .then($.proxy(this._got_kernelspecs, this));
  };
  ...
  KernelSelector.prototype._got_kernelspecs = function (data) {
    ...
    keys.map(function (key) {
      ...
      change_kernel_submenu.append(
        $("<li>")
          .attr("id", "kernel-submenu-" + ks.name)
          .append(
            $("<a>")
      ...
      new_notebook_submenu.append(
        $("<li>")
          .attr("id", "new-notebook-submenu-" + ks.name)
          .append(
            $("<a>")
              .attr("href", "#")
              .click(function () {
  ...
  KernelSelector.prototype._spec_changed = function (event, ks) {
    ...
    if (parent.children().length > 1) {
      ...
      parent.append(
        parent.children("li[class!='divider']").sort(function (a, b) {
          var da = $("a", a).text();
          var db = $("a", b).text();
          if (da === db) {
            return 0;
    ...
    if (ks === undefined) {
      ...
      if (selected.language && selected.language.length > 0) {
        available.map(function (name) {
          if (
            kernelspecs[name].spec.language.toLowerCase() ===
  ...
  KernelSelector.prototype._spec_not_found = function (event, data) {
    ...
    var no_kernel_msg = i18n.msg.sprintf(
      i18n.msg._(
    ...
    var body = $("<form>")
      ...
      .append($("<span>").text(no_kernel_msg))
      .append(select);
    ...
    var button_labels = [
      i18n.msg._("Continue Without Kernel"),
      i18n.msg._("Set Kernel"),
      i18n.msg._("OK"),
    ];
  ...
  KernelSelector.prototype.new_notebook = function (kernel_name) {
    ...
    */
    const url = new URL(window.location.href);
    url.searchParams.set("kernel", kernel_name);
    var w = window.open(url.href);
  ...
  KernelSelector.prototype.bind_events = function () {
    ...
    this.events.on(
      ...
      $.proxy(this._spec_not_found, this)
    );
