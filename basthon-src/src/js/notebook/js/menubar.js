...
define([
  ...
  MenuBar.prototype._bundle = function (bundler_id) {
    ...
    var url =
      utils.url_path_join(this.base_url, "bundle", notebook_path) +
      "?bundler=" +
      utils.encode_uri_components(bundler_id);
  ...
  MenuBar.prototype._nbconvert = function (format, download) {
    ...
    var url =
      utils.url_path_join(this.base_url, "nbconvert", format, notebook_path) +
      "?download=" +
      download.toString();
  ...
  MenuBar.prototype._size_header = function () {
     ...
     */
    console.warn(
      "`_size_header` is deprecated and will be removed in future versions." +
  ...
  MenuBar.prototype.bind_events = function () {
    ...
    this.element.find("#download_menu li").click(function (ev) {
      that._nbconvert(
        ev.target.parentElement.getAttribute("id").substring(9),
        true,
      );
    });
    ...
    this.events.on("trust_changed.Notebook", function (event, trusted) {
      if (trusted) {
        that.element
          ...
          .off("click")
          .find("a")
          .text(i18n.msg._("Trusted Notebook"));
      } else {
        that.element
          .find("#trust_notebook")
          ...
          .find("a")
          .text(i18n.msg._("Trust Notebook"));
      }
    });
  ...
  MenuBar.prototype._add_celltoolbar_list = function () {
    ...
    this.events.on("unregistered_preset.CellToolbar", function (event, data) {
      submenu
        .find("li[data-name='" + encodeURIComponent(data.name) + "']")
        .remove();
    });
  };
  ...
  MenuBar.prototype.update_restore_checkpoint = function (checkpoints) {
    ...
    if (!checkpoints || checkpoints.length === 0) {
      ul.append(
        $("<li/>")
          .addClass("disabled")
          .append($("<a/>").text(i18n.msg._("No checkpoints"))),
      );
      return;
    }
  ...
  MenuBar.prototype.update_nbconvert_script = function (langinfo) {
    ...
    el.find("a").text(
      langname + " (" + (langinfo.file_extension || "txt") + ")",
    );
  };
  ...
  MenuBar.prototype.add_kernel_help_links = function (help_links) {
    ...
    if (divider.length === 0) {
      ...
      divider = $("<li>").attr("id", "kernel-help-links").addClass("divider");
      ...
      cursor.after(
        $("<li>").append(
          $("<a>")
            ...
            .append(
              $("<i>").addClass("fa fa-external-link menu-icon pull-right"),
            )
