...
define([
  ...
  "use strict";
  function encodeURIandParens(uri) {
    return encodeURI(uri).replace("(", "%28").replace(")", "%29");
  }
  ...
  TextCell.prototype.create_element = function () {
    ...
    var input_area = $("<div/>")
      .addClass("input_area")
      .attr("aria-label", i18n.msg._("Edit Markup Text here"));
    this.code_mirror = new CodeMirror(
      input_area.get(0),
      this._options.cm_config,
    );
  ...
  TextCell.prototype.toJSON = function (gc_attachments) {
    gc_attachments = gc_attachments ?? false;

    let data = Cell.prototype.toJSON.apply(this);
    ...
    for (const key in this.attachments) {
      if (!gc_attachments || data.source.includes(key))
        data.attachments[key] = JSON.parse(
          JSON.stringify(this.attachments[key]),
        );
    }
  ...
  MarkdownCell.prototype.is_deletable = function () {
    return (
      !this.notebook.is_romd() && TextCell.prototype.is_deletable.call(this)
    );
  };

  MarkdownCell.prototype.is_editable = function () {
    return (
      !this.notebook.is_romd() && TextCell.prototype.is_editable.call(this)
    );
  };
  ...
  MarkdownCell.prototype.execute = function () {
    ...
    // next_breakpoint_cell already checks we are the last cell
    if (nb.is_breakpoint_cell(this)) nb.next_breakpoint_cell(this.cell_id);
  };
  ...
  MarkdownCell.prototype.set_heading_level = function (level) {
    ...
    source = source.replace(/^(#*)\s?/, new Array(level + 1).join("#") + " ");
  ...
  MarkdownCell.prototype.insert_inline_image_from_blob = function (
    blob,
    rename,
  ) {
    /**
    ...
    var key;
    if (rename || blob.name == null) {
      let id =
        globalThis.crypto?.randomUUID?.() ??
        Object.keys(that.attachments).length.toString();
      // uuid is a bit too long...
      id = id.split("-")[0];
      key = "image_" + id;
    } else {
    ...
    reader.onloadend = function () {
      ...
      if (blob.type != d[0]) {
        ...
        console.log(
          "File type (" + blob.type + ") != data-uri " + "type (" + d[0] + ")",
        );
      }
  ...
  MarkdownCell.prototype.bind_events = function () {
    ...
    this.element.dblclick(function () {
      ...
      if (that.notebook.is_romd()) return;
      ...
      if (data.items !== undefined) {
        for (var i = 0; i < items.length; ++i) {
          ...
          if (item.kind == "file" && attachment_regex.test(item.type)) {
            ...
            evt.stopPropagation();
            evt.preventDefault();
            that.insert_inline_image_from_blob(item.getAsFile(), true);
          }
        }
      }
    });
    ...
    this.code_mirror.on("drop", function (cm, evt) {
      ...
      for (var i = 0; i < files.length; ++i) {
        ...
        if (attachment_regex.test(file.type)) {
          ...
          evt.stopPropagation();
          evt.preventDefault();
          that.insert_inline_image_from_blob(file, false);
        }
      }
    });
  };
  ...
  RawCell.options_default = {
    ...
    placeholder: i18n.msg._(
      "Write raw LaTeX or other formats here, for use with nbconvert. " +
