...
define([
  ...
  function Notebook(selector, options) {
    ...
    this.set_sequenced = (v) => (this._sequenced = !!v);
    this.is_sequenced = () => this._sequenced;
    this._rest_sequence = [];
    this._run_all_sequenced = false;
    // romd extension (read-only markdown)
    this._romd = false;
    this.set_romd = (v) => (this._romd = !!v);
    this.is_romd = () => this._romd;
    ...
    // runall mechanism in sequenced context
    this.events.on("finished_execute.CodeCell", (event, data) => {
      if (this.is_sequenced()) {
        // breakpoint cell that do not add cell after exec
        // should break the runall sequence (even standard cell actually)
        if (this.find_cell_index(data.cell) === this.ncells() - 1)
          this._run_all_sequenced = false;
      }
    });
    ...
    if (marked) {
          ...
          async highlight(code, lang, info) {
            ...
            try {
              spec = await new Promise((resolve, reject) =>
                utils.requireCodeMirrorMode(lang, resolve, reject),
              );
            } catch (err) {
            ...
            const mode = CodeMirror.getMode({}, spec);
            if (!mode) {
              console.log(`No CodeMirror mode: ${lang}`);
              return code;
            }
        ...
        extensions: [
          ...
          // prevent marked from returning inline styles for table cells
          {
            ...
            renderer(token) {
              ...
              return `<${tag}${style}>${token.text}</${tag}>\n`;
            },
          },
        ],
      });
      ...
      const walkTokens = async (token) => {
        ...
        if (token.type !== "image" || !token.href.startsWith(prefix)) return;
        await this.basthonGUI.loaded(); // kernel and files are ready
        const kernel = this.basthonGUI.kernelSafe;
        const path = token.href.slice(prefix.length);
        try {
          token.href =
            "data:image/png;base64," +
            Base64.fromUint8Array(await kernel.getFile(path));
        } catch (e) {
          token.href = "";
        }
      };
  ...
  Notebook.prototype.handle_edit_mode = function (cell) {
    ...
    if (cell && this.mode !== "edit") {
      // [basthon] romd extension implementation
      if (this.is_romd() && cell.cell_type === "markdown") return;
      cell.edit_mode();
      this.mode = "edit";
  ...
  Notebook.prototype.to_code = function (index) {
    ...
    if (this.is_valid_cell_index(i)) {
      ...
      // [basthon] romd
      if (this.is_romd() && source_cell.cell_type === "markdown") return;
      if (
  ...
  Notebook.prototype.set_theme = function (theme) {
    ...
    marked.use({ langPrefix: `${new_class} language-` });
  ...
  Notebook.prototype.insert_image = function () {
    ...
    dialog.insert_image({
      callback: function (file) {
        cell.edit_mode();
        cell.insert_inline_image_from_blob(file, false);
      },
      notebook: this,
      keyboard_manager: this.keyboard_manager,
    });
  ...
  /**
   * Toggle fullscreen mode for the notebook.
   */
  Notebook.prototype.toggle_fullscreen = function () {
  ...
  Notebook.prototype._restart_kernel = function (options) {
    ...
    function restart_and_resolve() {
      that.kernel.restart(function () {
        ...
        that.events.one("kernel_ready.Kernel", () => {
          /* [Basthon] */
          // in sequenced mode, force notebook reloading
          // to avoid bypassing the sequence/breakpoint system.
          // this should not restart the kernel twice since
          // fromJSON check for execution count to be > 0
          if (that.is_sequenced()) that.fromJSON(that.toJSON());
          resolve_promise();
        });
      }, reject_promise);
    }
    ...
    // if no dialog, do kernel actions
  ...
  Notebook.prototype.execute_cell_and_select_below = function () {
    ...
    this.command_mode();
    if (
      cell_index === this.ncells() - 1 &&
      !this.is_breakpoint_index(cell_index)
    ) {
      this.insert_cell_below();
      this.select(cell_index + 1);
      this.edit_mode();
  ...
  Notebook.prototype.execute_all_cells = function () {
    if (this.is_sequenced()) this._run_all_sequenced = true;
  ...
  Notebook.prototype.fromJSON = function (data) {
    ...
    var cell_data = null;
    var new_cell = null;
    var after_breakpoint_cell = false;
    this._rest_sequence = [];
    this._run_all_sequenced = false;
    for (i = 0; i < ncells; i++) {
      cell_data = new_cells[i];
      if (this.is_sequenced() && after_breakpoint_cell) {
      ...
      if (this.is_breakpoint_cell(new_cell)) {
        after_breakpoint_cell = true;
      }
    }
    ...
    // [basthon] restart the kernel after loading new notebook
    // only if it is not the first notebook loaded in the session
    if ((this.basthonGUI.kernelSafe?.execution_count ?? 0) > 0)
      this._restart_kernel();
  };

  /**
   * [Basthon] Get a cell index from the last message id.
   */
  Notebook.prototype.last_msg_id_to_cell_index = function (msg_id) {
    var result = null;
    this.get_cell_elements().filter(function (index) {
      const cell = $(this).data("cell");
      if (cell.last_msg_id === msg_id) result = index;
    });
    return result;
  };

  /**
   * [Basthon] Get a cell index from the cell_id.
   */
  Notebook.prototype.cell_id_to_cell_index = function (cell_id) {
    var result = null;
    this.get_cell_elements().filter(function (index) {
      const cell = $(this).data("cell");
      if (cell.cell_id === cell_id) result = index;
    });
    return result;
  };

  /**
   * Test if a cell is a breakpoint one.
   */
  Notebook.prototype.is_breakpoint_cell = function (cell) {
    return (
      this.is_sequenced() &&
      ["code", "markdown"].includes(cell.cell_type) &&
      (cell?.metadata?.tags ?? []).includes("breakpoint")
    );
  };

  /**
   * Test if a cell index is a breakpoint one.
   */
  Notebook.prototype.is_breakpoint_index = function (index) {
    const cell = this.get_cell(index);
    return cell != null && this.is_breakpoint_cell(cell);
  };

  /**
   * Add remaining cells until next breakpoint cell.
   * cell_id is the id of the cell requesting the
   * continuation.
   */
  Notebook.prototype.next_breakpoint_cell = function (cell_id) {
    if (!this.is_sequenced()) return;
    const index = this.cell_id_to_cell_index(cell_id);
    // only last cell can request a continuation
    if (index !== this.ncells() - 1) return;
    // only a breakpoint cell can request a continuation
    if (!this.is_breakpoint_index(index)) return;
    let trusted = true;
    while (this._rest_sequence.length) {
      ...
      if (this.is_breakpoint_cell(new_cell)) break;
    }
    ...
    this.select(Math.min(index + 1, this.ncells() - 1));
    this.focus_cell();

    // keep going on execution
    if (this._run_all_sequenced)
      this.execute_cell_range(index + 1, this.ncells());
  };
  ...
  Notebook.prototype.toJSON = function () {
    ...
    // do not forget to append rest sequence in sequenced mode
    if (this.is_sequenced()) cell_array.push(...this._rest_sequence);
    var data = {
      cells: cell_array,
