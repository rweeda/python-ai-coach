...
define(["jquery", "base/js/events", "base/js/i18n"], function (
  $,
  events,
  i18n,
) {
  ...
  var _actions = {
    ...
    "split-cell-at-cursor": {
      ...
      help_index: "ea",
      handler: function (env) {
        const nb = env.notebook;
        // do not split a breakpoint cell
        if (nb.is_breakpoint_cell(nb.get_selected_cell())) return;
    ...
    "cut-cell": {
      ...
      handler: function (env) {
        ...
        // do not cut a breakpoint cell
        if (env.notebook.is_breakpoint_index(index)) return;
    ...
    "paste-cell-below": {
      ...
      handler: function (env) {
        const nb = env.notebook;
        const cell = nb.get_selected_cell();
        const is_last_cell = nb.find_cell_index(cell) === nb.ncells() - 1;
        // do not past below a non validated breakpoint cell
        if (nb.is_breakpoint_cell(cell) && is_last_cell) return;
    ...
    "insert-cell-below": {
      ...
      handler: function (env) {
        ...
        const cell = nb.get_selected_cell();
        const is_last_cell = nb.find_cell_index(cell) === nb.ncells() - 1;
        // do not insert after a non validated breakpoint cell
        if (nb.is_breakpoint_cell(cell) && is_last_cell) return;
    ...
    "change-cell-to-code": {
      ...
      handler: function (env) {
        const nb = env.notebook;
        // do not change breakpoint cell type
        if (nb.is_breakpoint_cell(nb.get_selected_cell())) return;
    ...
    "move-cell-down": {
      ...
      handler: function (env) {
        const nb = env.notebook;
        const cell = nb.get_selected_cell();
        // do not move a breakpoint cell
        if (nb.is_breakpoint_cell(cell)) return;
        const cell_index = nb.find_cell_index(cell);
        // do not move below a non validated breakpoint cell
        if (
          cell_index === nb.ncells() - 2 &&
          nb.is_breakpoint_index(cell_index + 1)
        )
          return;
    ...
    "merge-cell-with-previous-cell": {
      ...
      handler: function (env) {
        const nb = env.notebook;
        const index = nb.get_selected_index();
        // do not merge breakpoint cell
        if (nb.is_breakpoint_index(index) || nb.is_breakpoint_index(index - 1))
          return;
    ...
    "merge-selected-cells": {
      ...
      handler: function (env) {
        const nb = env.notebook;
        const selected_cells = nb.get_selected_cells_indices();
        // do not merge a breakpoint cell
        for (let index of selected_cells)
          if (nb.is_breakpoint_index(index)) return;
    ...
    "merge-cells": {
      ...
      handler: function (env) {
        ...
        if (selected_cells.length === 1) {
          const index = selected_cells[0];
          // do not merge a breakpoint cell
          if (nb.is_breakpoint_index(index + 1)) return;
          env.notebook.merge_cell_below();
        } else {
          env.notebook.merge_selected_cells();
        }
  ...
  var custom_ignore = {
    ...
    "toggle-fullscreen": {
      cmd: i18n.msg._("toggle fullscreen"),
      help: i18n.msg._("Plein écran"),
      icon: "fa-expand",
      help_index: "fzz",
      handler: function (env, event) {
        env.notebook.toggle_fullscreen();
        env.notebook.focus_cell();
        if (event) {
          event.preventDefault();
        }
  ...
  var _prepare_handler = function (registry, subkey, source) {
    ...
    registry["jupyter-notebook:" + subkey].help =
      source[subkey].help || subkey.replace(/-/g, " ");
    registry["jupyter-notebook:" + subkey].help_index =
      source[subkey].help_index;
    ...
    for (k in _actions) {
      if (_actions.hasOwnProperty(k)) {
        ...
        (function (key, handler) {
          final_actions["jupyter-notebook:" + key].handler = function (
            env,
            event,
          ) {
            handler(env);
