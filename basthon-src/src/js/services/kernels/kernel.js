...
define([
  ...
  Kernel.prototype.init_iopub_handlers = function () {
    ...
    this.register_iopub_handler(
      "status",
      $.proxy(this._handle_status_message, this),
    );
    this.register_iopub_handler(
      "clear_output",
      $.proxy(this._handle_clear_output, this),
    );
    this.register_iopub_handler(
      "execute_input",
      $.proxy(this._handle_input_message, this),
    );

    for (var i = 0; i < output_msg_types.length; i++) {
      this.register_iopub_handler(
        output_msg_types[i],
        $.proxy(this._handle_output_message, this),
      );
    }
  };
  ...
  Kernel.prototype.start = function (params, success, error) {
    ...
    this.basthonKernelAvailable.then(() => on_success({ id: 0 }));
    return;
  ...
  Kernel.prototype.interrupt = function (success, error) {
    ...
    this.basthonKernelAvailable.then((kernel) => {
      kernel.restart();
      kernel.ready().then(() => on_success({ id: 0 }));
    });
    return;
  ...
  Kernel.prototype.restart = function (success, error) {
    ...
    this.basthonKernelAvailable.then((kernel) => {
      kernel.restart();
      kernel.ready().then(() => on_success({ id: 0 }), on_error);
    });
    return;
  ...
  Kernel.prototype._on_success = function (success) {
    ...
    return function (data, status, xhr) {
      ...
      that.kernel_url = utils.url_path_join(
        that.kernel_service_url,
  ...
  Kernel.prototype._kernel_created = function (data) {
     ...
     */
    this.id = data.id;
    this.kernel_url = utils.url_path_join(
  ...
  Kernel.prototype.send_shell_message = function (
    msg_type,
    content,
    callbacks,
    metadata,
   ...
   */
  Kernel.prototype.output_callback_overrides_push = function (
    msg_id,
    callback_id,
  ) {
  ...
  Kernel.prototype._handle_ws_message = function (e) {
    ...
    this._msg_queue = this._msg_queue
      ...
      .catch(function (error) {
        console.error(
          "Couldn't process kernel message " + error.toString(),
          error,
        );
      });
  };
  ...
  Kernel.prototype._handle_shell_reply = function (reply) {
    ...
    if (content.payload && shell_callbacks.payload) {
      promise = promise.then(function () {
        return that._handle_payloads(
          content.payload,
          shell_callbacks.payload,
          reply,
        );
      });
    }
   ...
   */
  Kernel.prototype._handle_payloads = function (
    payloads,
    payload_callbacks,
    msg,
  ) {
  ...
  Kernel.prototype._handle_output_message = function (msg) {
    ...
    if (
      ["display_data", "update_display_data", "execute_result"].indexOf(
        msg.header.msg_type,
      ) > -1
    ) {
      ...
      if (display_id) {
        ...
        if (
          this._display_id_to_parent_ids[display_id].indexOf(callback_id) === -1
        ) {
